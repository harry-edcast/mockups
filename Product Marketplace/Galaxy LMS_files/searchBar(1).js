...
function funcBuildList(val, inp, strRawData) {
	...
	for (var i = 0; i < objTraining.data.length; i++) {
		var strTitle = objTraining.data[i].title;
		var strID = objTraining.data[i].id
		/*create a DIV element for each matching element:*/
		var b = document.createElement("DIV");
    b.setAttribute('data-loid',strID);
    b.addEventListener('click',funcOpenLO);
		b.innerHTML = '<span>' + strTitle + "</span>";
    a.appendChild(b);
	}
}

function funcOpenLO(e) {
  console.log(e);
  if(e.target.getAttribute('data-loid') && e.target.getAttribute('data-loid') != '') {
    var strLOID = e.target.getAttribute('data-loid');
  } else {
    var strLOID = e.target.parentElement.getAttribute('data-loid');
  }
  //window.open(`/LMS/LoDetails/DetailsLo.aspx?loid=${strLOID}`,'_top');
  window.open(`https://cornerstone.pxp.csod.com/app/content?c=CSOD_${strLOID}`,'_top');
}
...
function funcSetupSearch() {
	...
	//document.getElementById('aspnetForm').action = "https://cornerstone.pxp.csod.com/app/search/content/learning";
	//document.getElementById('searchbox2').id = "query";
}
...
function autocomplete(inp) {
  ...
  inp.addEventListener("keydown", function(e) {
	...
	//window.open("https://cornerstone.pxp.csod.com/app/search/content/learning?st=" + encodeURIComponent(document.getElementById("searchbox2").value),"_top")
        if (currentFocus > -1) {
