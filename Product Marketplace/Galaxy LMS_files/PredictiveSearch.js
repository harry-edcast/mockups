function PredictiveSearch(elementId, handler, tokens, onPopulateQuery, onItemSelected, categoryItemsLimit, loTypeLimit, minChars) {
    ...
    self.initialize = function () {
        if (self.element != null) {
            ...
            self.dropdown.className = "search-visually-hidden";
            self.dropdown.setAttribute('aria-live', 'polite');
            self.dropdown.setAttribute('role', 'status');
            self.dropdown.id = "searchResultCount";
                ...
                if (xhr.readyState == 4 && xhr.status == 200) {
                    if (self.lastQueryTime == thisQueryTime) {
                        ...
                        if (xhr.responseText) {
                            ...
                            document.getElementById("searchResultCount").innerHTML = count == 0 ? "No results availaible" : count + " results availaible";
                        }
