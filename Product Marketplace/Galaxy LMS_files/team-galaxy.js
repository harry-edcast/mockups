...
function getCheckInsToken() {
  ...
  xhrConvo.onreadystatechange = function() {
      ...
      setTimeout(function() {getCheckInsToken() }, 500);
    }
...
function getParticipants() {
  ...
  xhr.onreadystatechange = function() {
      ...
      setTimeout(function() {getParticipants() }, 500);
    }
...
function buildParticipants(strRawData) {
  ...
  for(var i=0;i<objParticipants.directReports.length; i++) {
    ...
    elmImageContainer.innerHTML = '<div class="image"><a href="/phnx/driver.aspx?routename=Social/UniversalProfile/Snapshot&TargetUser=' + objParticipants.directReports[i].id + '" title="Open Hub"><img src="' + strPicture + '" alt="Open Hub" title="Open Hub" rel="tooltip"></a></div>';
    ...
    elmInfoContainer.innerHTML = '<div class="userName"><a href="/phnx/driver.aspx?routename=Social/UniversalProfile/Snapshot&TargetUser=' + objParticipants.directReports[i].id + '" title="Open Hub">' + objParticipants.directReports[i].firstName + ' ' + objParticipants.directReports[i].lastName + '</a><br><span class="userPosition" title="' + objParticipants.directReports[i].position + '" alt="' + objParticipants.directReports[i].position + '">' + objParticipants.directReports[i].position + '</span></div><div class="userActions"><a href="/ui/perf-check-ins/Check-Ins/users/' + objParticipants.directReports[i].id + '" title="Check-Ins"> Check-In</a> |<a href="/phnx/driver.aspx?routename=Social/UniversalProfile/Snapshot/Reviews&TargetUser=' + objParticipants.directReports[i].id + '" title="Dev Convos"> Dev Convos</a> <br> <a href="/phnx/driver.aspx?routename=Social/UniversalProfile/Snapshot/Goals&TargetUser=' + objParticipants.directReports[i].id + '" title="Open Goals"> Goals</a> | <a href="/phnx/driver.aspx?routename=Social/UniversalProfile/Feedback&TargetUser=' + objParticipants.directReports[i].id + '" title="Open Feedback"> Feedback</a></div>';
